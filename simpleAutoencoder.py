...
class autoencoder():
	def trainAutoencoder(dataset, timesteps, input_dim, bottleneckDim, lossEvaluation, optimizer, epochs, batchSize, verbose=False):
	...
	for limitAux in range(12,13): #12
		...
		for bottleneckDim in range (2,3): #4
			...
			for look_back in range(2,3): #2
				...
				# train model
				Model = autoencoder.trainAutoencoder(dataset = dataset, timesteps = timesteps, input_dim = input_dim, bottleneckDim = bottleneckDim, lossEvaluation=lossEvaluation, optimizer=optimizer, epochs=epochs, batchSize=batchSizeModel, verbose=False)
